<div align="center">

# 🔫 Ohaka Shooter (OSU!")

**A fast-paced 2D top-down shooter game built with GameMaker Studio 2.**

[![GameMaker Studio 2](https://img.shields.io/badge/GameMaker-Studio%202-brightgreen.svg?logo=gamemaker&style=for-the-badge)](https://gamemaker.io/)
[![GML](https://img.shields.io/badge/Language-GML-blue.svg?style=for-the-badge)](#)
[![Status](https://img.shields.io/badge/Status-Active-success.svg?style=for-the-badge)](#)

</div>

---

## 📖 Overview

**Ohaka Shooter (OSU!")** features intense bullet-hell gameplay, multiple challenging bosses, and an innovative **"Combo Chain"** system that rewards consecutive kills with powerful temporary abilities. Dive into the arena, dodge incoming fire, and survive as long as you can!

## ✨ Features

- 🎯 **Top-Down Shooter Gameplay**: Fast-paced action with fluid WASD movement and Mouse aiming/shooting.
- 🔥 **Combo Chain System**:
  - **Consecutive Kills**: Eliminating enemies quickly extends your combo chain.
  - **Milestone Rewards**: Reaching certain combo thresholds unlocks temporary power-ups:
    - 🛡️ **Invincibility**: Short invulnerability period.
    - 💥 **Double Shot**: Fire two bullets at once.
    - ⚡ **Speed Boost**: Temporary movement speed increase.
    - 💣 **Bomb**: Clears all enemies on screen.
- 👹 **Epic Boss Battles**:
  - **Skull**: Shoots projectiles in various bullet-hell patterns.
  - **Worm**: Chases the player with continuous melee attacks.
  - **Spider**: Creates sticky webs and summons smaller spiderlings.
  - **Queen**: The ultimate final boss equipped with a massive bullet arsenal.
- 💊 **Power-ups & Enhancements**: Health pickups and temporary weapon upgrades dropped during combat.

## 🎮 How to Play

### Controls
| Key/Input | Action |
| :--- | :--- |
| `W`, `A`, `S`, `D` | Move the character |
| `Mouse Move` | Aim weapon |
| `Left Click` | Shoot |
| `R` | Restart game (on Game Over) |

### Gameplay Mechanics
1. **Survive Waves**: Eliminate all enemies in each wave to proceed to the next difficulty level.
2. **Build Combos**: Kill enemies consecutively to build up your combo chain. Beware: taking damage or waiting too long will reset the counter!
3. **Utilize Power-ups**: Collect health packs and strategically use weapon boosts to clear tough situations and survive longer.

## 🎨 Assets & Art
- **Character**: Animated "Jumping" character sprite.
- **Environment**: Modular room system with interactive barriers for varied level design.
- **Visual Effects**: Polished bullet impacts, explosions, and power-up visual indicators.
> *Note: All assets were generated using Midjourney v6.*

## 💻 Development

- **Engine**: GameMaker Studio 2
- **Language**: GML (GameMaker Language)
- **Architecture**: Object-oriented approach with clear separation of concerns (Players, Enemies, Projectiles, and Power-ups).

### Project Structure
- `Objects/` - Contains all game entities.
- `Scripts/` - Core game logic, state machines, and utility functions.
- `Rooms/` - Level layouts and boss arenas.
- `Sprites/` - Visual assets for all elements.

## 🤝 Credits
- **Art & Assets**: Generated via Midjourney v6.
- **Programming & Architecture**: Developed with the assistance of Antigravity AI.

---
<div align="center">
  <i>If you find this project interesting, consider giving it a ⭐!</i>
</div>
